...
export default function Payments({ user }) {
  ...
  if (process.env.REACT_APP_URL) {
    createInvoiceUrl = `${process.env.REACT_APP_URL}/backend/payments/createInvoice`;
  } else {
  ...
  let invoicesUrl;
  if (process.env.REACT_APP_URL) {
    invoicesUrl = `${process.env.REACT_APP_URL}/backend/payments/payments`;
  } else {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      ...
      setInvoices(
        response.payments.filter((invoice) => invoice.payee === user.username)
      );
    }
    fetchMyAPI();
  }, [invoicesUrl, user]);
  ...
  let parentsUrl;
  if (process.env.REACT_APP_URL) {
    parentsUrl = `${process.env.REACT_APP_URL}/backend/users/parents`;
  } else {
  ...
  let paymentUrl;
  if (process.env.REACT_APP_URL) {
    paymentUrl = `${process.env.REACT_APP_URL}/backend/payments/checkout`;
  } else {
  ...
  const payAllButton = () => {
    if (invoices.length === 0) {
      return (
        <form
          action={`${paymentUrl}many/${user.username}/${total}/${user.phone}`}
          method="POST"
        >
          <button
            disabled
      ...
      <div className="flex flex-col px-8 mx-auto space-y-12 max-w-7xl xl:px-12">
          ...
          <div className="flex flex-col mb-8 animated fadeIn sm:flex-row">
            ...
            <div className="flex flex-col justify-center mt-5 mb-8 md:mt-0 sm:w-1/2 md:w-7/12 sm:pr-16">
              ...
              <p className="mt-5 text-lg text-gray-700 text md:text-left">
                Here you can pay for your child's tuition, sports fee's,
                activities and more! You can view your statements below.
              </p>
            </div>
          </div>
        </div>
        <div className="flex flex-col mb-8 animated fadeIn sm:flex-row">
          <div className="flex items-center mb-8 sm:w-1/2 md:w-5/12 sm:order-last">
            <h3 className="mt-2 text-2xl sm:text-left md:text-4xl center">
              Pay your total outstanding payments
            </h3>
          </div>
          <div className="flex flex-col justify-center mt-5 mb-8 md:mt-0 sm:w-1/2 md:w-7/12 sm:pr-16">
        ...
        <div className="flex flex-col mb-8 animated fadeIn sm:flex-row">
          <div className="flex items-center mb-8 sm:w-1/2 md:w-5/12">
            <h3 className="mt-2 text-2xl sm:text-left md:text-4xl">
              Pay your payments individually
            </h3>
          </div>
                  ...
                  <center>
                    ...
                    <div className="shadow-lg bg-white rounded-md overflow-hidden max-w-xs mx-auto">
                      <div className="w-2/3 p-4">
                        ...
                        <div>
                          <form
                            action={`${paymentUrl}/${invoice._id}/${invoice.title}/${invoice.price}/${user.phone}`}
                            method="POST"
                          >
                            <button
                              type="submit"
        ...
        <div className="relative">
          ...
          <div className="flex flex-col mb-8 animated fadeIn sm:flex-row">
            ...
            <div className="flex flex-col justify-center mt-5 mb-8 md:mt-0 sm:w-1/2 md:w-7/12 sm:pr-16">
              ...
              <p className="mt-5 text-lg text-gray-700 text md:text-left">
                Here you can create an invoice for parents to pay
              </p>
            </div>
          </div>
          <div className="bg-white rounded-2xl border shadow-x1 p-10 max-w-lg">
            <div className="flex flex-col items-center space-y-4">
              ...
              >
                ...
                {parents.map((parent) => (
                  <option key={parent.username} value={parent.username}>
                    {parent.username}
                  </option>
                ))}
              </select>
