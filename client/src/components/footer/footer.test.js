...
describe("Footer", () => {
  it("contains company information", () => {
    ...
    expect(linkTwo.getAttribute("href")).toBe(
      "https://www.gov.uk/government/organisations/department-for-education"
    );

    const linkThree = screen.getByRole("link", { name: "MERN Stack" });
    expect(linkThree.getAttribute("href")).toBe(
      "https://www.geeksforgeeks.org/mern-stack/"
    );

    const linkFour = screen.getByRole("link", { name: "About us" });
    expect(linkFour.getAttribute("href")).toBe(
      "https://github.com/jmcnally17/EdUp#readme"
    );
  });
});
